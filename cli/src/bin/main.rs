...
use cli::{
    ...
    ncn_portal_handler::NcnPortalCliHandler,
    CliConfig,
};
use env_logger::Env;
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let ncn_portal_program_id = if let Some(restaking_program_id) = &args.ncn_portal_program_id {
        Pubkey::from_str(restaking_program_id)?
    } else {
        ncn_portal_client::programs::NCN_PORTAL_ID
    };

    match args.command.expect("Command not found") {
        ProgramCommand::NcnPortal { action } => {
            NcnPortalCliHandler::new(cli_config, ncn_portal_program_id)
                .handle(action)
                .await?;
        }
    }
